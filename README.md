...
- The current `develop` state now goes one boundary deeper and asks the native bridge to acknowledge those bounded pilot payloads in dry-run mode, which makes the bridge-side integration gap more explicit without pretending that force execution already exists.
