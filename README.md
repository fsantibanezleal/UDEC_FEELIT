...
| Public port | `8101` |
| Release-synced version | `2.18.008` |
...
| Current validation surface | `116` automated tests passing plus browser smoke validation across the `5` routed pages |
...
The manager route is the authoring and registry surface for structured `haptic_workspace` descriptors. It now combines registry maintenance, descriptor preview, structured editing, and lifecycle actions without turning absolute local paths into the primary reading surface.
...
- `Haptic Workspace Manager`: creates and registers structured `haptic_workspace` descriptors rooted in external folders, surfaces registry diagnostics when registered descriptors are missing or invalid, defaults to descriptor-label views instead of exposing absolute local paths, and now supports descriptor preview, structured metadata/root editing, asset-aware library previews, safe library-item add/update/reorder/remove actions, library rescan, unregister, plus conservative invalid-descriptor repair actions.
...
- the workspace manager now covers descriptor preview, structured editing, asset-aware library curation, and the core descriptor lifecycle, but it still does not provide bulk validation workflows, richer bundle-specific previews, or asset-level repair guidance beyond descriptor normalization
