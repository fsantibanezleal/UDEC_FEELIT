...
private:
	bool		m_BrailleTextReady;
	int*		m_Elementos;
	unsigned	m_numElementos;
...
public:
    ...
    // Initialize
    int init(double a_cubeSize, double a_stiffness);
	...
	stVertex	HapticRenderCaracterBraille(stVertex& vFuerzaResultante, int vIdentificador,stVertex vDimensionesUniverso,stVertex vPosicionActual,stVertex vRadioXYZ,stVertex vSeparacionXYZCaracteres,stVertex vSeparacionXYZNodos);
	void		HapticDrawNodoBraille(stVertex& vFuerzaResultante, stVertex vPosicion,stVertex vRadio);
	void		HapticVectorBrailleUpdate(int* vElementos, unsigned vNumElementos);
private:
    ...
    void cubeContact();
	void BrailleContact();
	stVertex cubeContact(stVertex vCentro, stVertex vRadio);
