...
class HapticBridgeCommandExecutionSnapshot(BaseModel):
    """Describe bounded native execution of one pilot command payload."""
...
def execute_native_bridge_command(
    executable_path: str | None,
    *,
    backend_slug: str,
    command_payload: dict[str, Any],
    ...
    """Send one bounded pilot command to the native bridge for first-step execution."""
    if not executable_path:
    ...
    try:
        ...
        return HapticBridgeCommandExecutionSnapshot(
            state="launch-failed",
            summary=f"Bridge command execution failed to launch: {error}",
            executable_path=str(bridge_path),
            backend_slug=backend_slug,
            command_slug=str(command_payload.get("command_slug", "")) or None,
        )
    ...
    if completed.returncode != 0 and not completed.stdout.strip():
        ...
        return HapticBridgeCommandExecutionSnapshot(
            ...
            summary=f"Bridge command execution exited with code {completed.returncode}: {stderr}",
            executable_path=str(bridge_path),
            backend_slug=backend_slug,
            command_slug=str(command_payload.get('command_slug', '')) or None,
        )
    ...
    try:
        ...
        return HapticBridgeCommandExecutionSnapshot(
            state="invalid-json",
            summary=f"Bridge command execution returned invalid JSON: {error}",
            executable_path=str(bridge_path),
            backend_slug=backend_slug,
            command_slug=str(command_payload.get("command_slug", "")) or None,
        )

    summary = str(payload.get("summary", "Bridge command execution returned no summary."))
    state = str(payload.get("status", "unknown"))
